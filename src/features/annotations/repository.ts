...
export const excerptRepository = {
  ...
  /**
   * Lets an excerpt's captured quote and/or note be edited in place —
   * e.g. paraphrasing a highlight directly, or trimming stray text a
   * PDF text-layer selection picked up — rather than requiring a new
   * excerpt to be created every time.
   */
  async updateContent(id: string, quote: string, userNote: string | null): Promise<void> {
    await storageClient.execute(
      "UPDATE excerpts SET quote = $1, user_note = $2 WHERE id = $3",
      [quote, userNote, id],
    );
  },
