...
function updateFromForm(values: FormState): {
  title: string | null;
  authors: string[] | null;
  doi: string | null;
  journal: string | null;
 ...
 * meta tags for pages without one — see `lookupCitation` in
 * `features/library/service.ts`).
 */
export function MetadataSection({ paper, onSaved }: MetadataSectionProps) {
  ...
  async function save(basePaper: Paper, values: FormState) {
    const update = updateFromForm(values);
  ...
  /**
   * Builds the citation from the form's *current* state (`form`, via
   * `updateFromForm` — the same parsing `save()` uses) rather than the
   * `paper` prop, which can be a keystroke or a debounce-interval
   * behind whatever's actually typed. The merge onto `paper` below
   * only backfills fields the citation formatter needs that aren't
   * part of this form (id/filePath/etc. — irrelevant to APA7, but
   * `formatApa7Citation` takes a full `Paper`).
   */
  async function handleCopyCitation() {
    const citation = formatApa7Citation({ ...paper, ...updateFromForm(form) });
    try {
      ...
      // nothing more useful to do here than silently not confirm.
    }
  }
  ...
  return (
    <div className="metadata-form">
      ...
      <div className="metadata-form__citation">
        <span className="metadata-form__citation-label">APA7 Citation</span>
        <p className="metadata-form__citation-preview">
          {formatApa7Citation({ ...paper, ...updateFromForm(form) })}
        </p>
        <button
          className="metadata-form__citation-copy"
          onClick={handleCopyCitation}
          type="button"
        >
          {citationCopied ? "Copied!" : "Copy APA7 Citation"}
        </button>
      </div>
    </div>
  );
